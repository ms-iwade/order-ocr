...
- itemCode に使う列の優先順位は、「JANCD / JANコード」→「コード / 商品コード / 品番コード」→ null です。
- 「発注NO」「発注No」「発注番号」「注文No」「受注No」などの管理番号列は、itemCode に使ってはいけません。
- 「発注備考」「備考」「摘要」「記事」「メモ」などの自由記述列や、そのセル内のコードらしく見える文字列は、itemCode に使ってはいけません。
- 「発注NO」と「コード」が両方ある場合は、必ず同じ行の「コード」列の値を itemCode に採用してください。
- 「コード」列と「発注NO」列の値が似ていても、長さが違っていても、見やすさに差があっても、itemCode には「コード」列だけを使ってください。
- 「商品コード」「コード」「品番コード」などの商品識別用の列に値がある場合は、同じ行の発注備考より常にその商品コードを優先してください。
...
- 先頭の 0 を落とす、連続する 0 の個数を減らす、見慣れた品番に寄せて桁を短くする、といった正規化や要約をしてはいけません。
- たとえば「10203」と見える場合に「123」と短縮してはいけません。
- たとえば「000123」は「123」にせず、「1000203」は「100203」や「10203」にせず、原本どおりに出力してください。
...
- 同じ行の発注備考や摘要にコードらしい記載があっても、それを itemCode の代わりに使ってはいけません。
...
手順5: 最後にページごと・行ごとの対応関係を見直してから出力してください。
- itemCode を最終出力する前に、「発注NO系の列を誤って使っていないか」「0 の個数を減らしていないか」を必ず再確認してください。
- itemCode を最終出力する前に、「発注備考や摘要の値を誤って使っていないか」も必ず再確認してください。
  ...
  "pages": [
    {
      ...
      "lineItems": [
        {
          "itemCode": "品番・コード列の値（JANCDがあればそちらを優先。発注NOや発注備考ではなく商品コード列を採用し、たとえば000123は123にせず、100103は10013にせず、途中の0の個数と位置を保ったまま1文字も省略せず出力）",
          "quantity": 発注数,

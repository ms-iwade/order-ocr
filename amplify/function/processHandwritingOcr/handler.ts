...
表の各行について、まず表の罫線内にある備考欄または出荷日回答欄のセル内に手書きで書かれた日付を抽出してください。
さらに、それらの欄に納期記載が見当たらない場合に限り、ページ下部や表の下側余白付近にある、注文全体に対する共通納期と判断できる手書き日付を候補として採用してよいです。
納期の候補としては手書き文字だけを対象にし、対象欄内にある印字文字は日付であっても完全に無視してください。
印刷された既定値、帳票の印字、対象欄以外の手書き、数量修正、関係ないメモは無視してください。
...
- 対象セル内に印字された日付や文字列があっても、それは候補にせず、手書きで追記・上書きされた文字だけを見てください。
...
- 印字された日付や数字列は、どれだけ鮮明でも deliveryDate の候補にしてはいけません。
...
- itemCode を特定するために列見出しや同じ行の商品コード欄などの印字を参照して構いませんが、deliveryDate の判定には印字文字を使ってはいけません。
...
- 印字された日付しか存在しない場合も、handwrittenItems を空配列にしてください。
